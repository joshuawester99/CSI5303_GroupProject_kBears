...
public class Reservation_Listing extends javax.swing.JFrame {
    ...
    public Reservation_Listing(User inputGuest) {
        this.guest = inputGuest;
        bookings = Main.masterController.getGuestBookings(guest);
        initComponents();

        model = (DefaultTableModel) room_list.getModel();

        for (Booking booking: bookings) model.addRow(new Object[] {booking.getRoom().getRoomNumber(), booking.getStartDate(), booking.getEndDate()});
        ...
        room_list.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
            ...
            public void valueChanged(ListSelectionEvent e) {
                ...
                LocalDate startDate = (LocalDate) model.getValueAt(selectedRow, 1);
                LocalDate endDate = (LocalDate) model.getValueAt(selectedRow, 2);

                for (Booking booking: bookings){
                    if (roomNumber.equals(booking.getRoom().getRoomNumber()) && startDate.equals(booking.getStartDate()) && endDate.equals(booking.getEndDate())){
                        selectedBooking = new Booking(booking);
                        Reservation_Display reservationDisplayFrame = new Reservation_Display(selectedBooking);
                        reservationDisplayFrame.setVisible(true);
                        reservationDisplayFrame.pack();
                        reservationDisplayFrame.setLocationRelativeTo(null);
                        Reservation_Listing.this.dispose();
                    }
                }
            }
        });
