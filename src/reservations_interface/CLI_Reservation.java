...
public class CLI_Reservation {
    ...
    public void makeReservation() throws IOException {
        ...
        long daysBetween = endDate.until(startDate, ChronoUnit.DAYS);
        long totalRate = chosenRoom.getRate() * daysBetween;

        if (Main.masterController.checkUserInstance().equals("Guest")){
            Main.masterController.bookRoom(chosenRoom, startDate, endDate, (Guest) Main.masterController.getCurrentUser(), totalRate);
        } else {
            Guest inputGuest = getGuest();
            Main.masterController.bookRoom(chosenRoom, startDate, endDate, inputGuest, totalRate);
